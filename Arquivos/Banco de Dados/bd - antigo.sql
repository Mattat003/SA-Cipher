...
ALTER TABLE compras ADD CONSTRAINT fk_jogo_id FOREIGN KEY (jogo_id) REFERENCES jogo(pk_jogo)
    ON DELETE RESTRICT
    ON UPDATE CASCADE;
ALTER TABLE jogo ADD COLUMN disponivel_locacao BOOLEAN DEFAULT TRUE;
ALTER TABLE jogo ADD COLUMN imagem_jogo VARCHAR(255) NULL;
ALTER TABLE jogo ADD COLUMN url_jogo VARCHAR(255) NULL;

ALTER TABLE biblioteca_usuario
ADD COLUMN jogo_id INT NULL,
ADD CONSTRAINT fk_biblioteca_jogo FOREIGN KEY (jogo_id) REFERENCES jogo(pk_jogo);

-- Agora, você pode remover nome_jogo, imagem_jogo, url_jogo se quiser (opcional):
ALTER TABLE biblioteca_usuario
DROP COLUMN nome_jogo,
DROP COLUMN imagem_jogo,
DROP COLUMN url_jogo;
/*=================*/
...
select * from idioma;
select * from jogo;
select * from locacoes_pendentes;
select * from modo;
select * from plataforma;
