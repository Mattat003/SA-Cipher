...
$busca_realizada = false; // Flag para indicar se uma busca específica foi realizada
...
try {
    ...
    $sql = "SELECT pk_usuario, nome_user, email_user FROM usuario"; // Removido 'perfil' daqui
    $params = [];

    // Se o formulário foi enviado E a busca não está vazia, filtra
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['busca']) && !empty(trim($_POST['busca']))) {
        $busca = trim($_POST['busca']);
        $busca_realizada = true; // Uma busca específica foi feita
...
<head>
    ...
    <style>
        ...
        form {
            ...
            align-items: center;
            padding: 15px;
            background: #e6e1f4; /* Fundo mais claro para o formulário */
            border-radius: 8px;
        ...
        form input[type="text"]:focus {
            border-color: #510d96;
            box-shadow: 0 0 0 3px rgba(81, 13, 150, 0.2);
            outline: none;
        }

        form button {
            background: #510d96; /* Cor de botão primária */
            color: #fff;
            padding: 12px 25px;
            border: none;
            border-radius: 5px;
        ...
        form button:hover {
            background: #2c056e; /* Cor mais escura no hover */
        ...
        @media (max-width: 768px) {
            ...
            form {
                flex-direction: column; /* Empilha os elementos do formulário em telas menores */
                align-items: stretch;
            }
            form input[type="text"], form button {
                width: 100%;
                max-width: none; /* Remove a largura máxima para preencher 100% */
            }
            table th, table td {
                padding: 10px;
                font-size: 0.85em;
    ...
    <div class="container">
            ...
            <table>
                <thead>
                    <tr>
                        ...
                        <th>Nome</th>
                        <th>Email</th>
                        <!-- Removido 'Perfil' daqui temporariamente -->
                        <th>Ações</th>
                    </tr>
                </thead>
                <tbody>
        ...
        <?php else : ?>
            <p>
                <?php
                if ($busca_realizada) {
                    echo "Nenhum usuário encontrado com os critérios de busca.";
                } else {
                    echo "Nenhum usuário cadastrado no sistema ou erro ao carregar.";
                }
                ?>
            </p>
        <?php endif; ?>
