...
// Verifica se o adm está logado e tem perfil de adm (1) ou funcionário (2)
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['busca']) && !empty(trim($_POST['busca']))) {
        ...
        $sql = "SELECT pk_adm, nome_adm, email_adm FROM adm";
        $params = [];
        if (is_numeric($busca)) {
            $sql .= " WHERE pk_adm = :busca";
            $params[':busca'] = $busca;
        } else {
            $sql .= " WHERE nome_adm LIKE :busca_nome";
            $params[':busca_nome'] = "%".$busca."%";
        }
        $sql .= " ORDER BY nome_adm ASC";
        $stmt = $pdo->prepare($sql);
        $stmt->execute($params);
        $adms = $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
} catch (PDOException $e) {
    echo "<p style='color: red; text-align: center;'>Erro ao carregar administradores: " . htmlspecialchars($e->getMessage()) . "</p>";
    $adms = [];
}
?>
