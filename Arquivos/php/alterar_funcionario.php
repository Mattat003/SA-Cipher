...
<body>
    ...
    <form action="alterar_funcionario.php" method="POST">
        <label for="busca_adm">Digite o ID ou Nome do Funcionário:</label>
        <input type="text" id="busca_adm" name="busca_adm" required>
        <button type="submit">Buscar</button>
    </form>

    <?php if ($adm): ?>
        <!-- Formulário de alteração -->
        <form action="processa_alteracao_adm.php" method="POST">
            ...
            <select id="fk_cargo" name="fk_cargo" required>
                ...
                <?php foreach ($cargos as $cargo): ?>
                    <option value="<?= htmlspecialchars($cargo['pk_cargo']) ?>" 
                        <?= $adm['fk_cargo'] == $cargo['pk_cargo'] ? 'selected' : '' ?>>
